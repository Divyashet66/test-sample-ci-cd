	    ...
	    stage('Build Docker Image') {
		    steps {
				...
				sh 'sudo add-apt-repository ppa:cncf-buildpacks/pack-cli'
			      ...
			      sh 'pack build node-app-22 --builder gcr.io/buildpacks/builder:google-22'
				sh 'docker tag node-app-22 us-central1-docker.pkg.dev/tech-rnd-project/network18/node-app-22'
		    ...
		    steps {
			    script {
				    echo "Push Docker Image"
				        sh 'gcloud auth configure-docker ar_region'
				        sh "sudo docker push us-central1-docker.pkg.dev/tech-rnd-project/network18/node-app-22"
